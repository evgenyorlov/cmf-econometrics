...
# Applied econometrics 3, Generalized hyperbolic distribution Орлов
# Евгений, 20.11.2014
...
plot(rts.emp_dens, type = "l", lwd = 3, main = "Fitted density functions", xlab = "Return")
...
legend("topleft", c("emp_dens", "gauss", "t", "nig", "vg", "hyp", "ghyp"), col = c("black", 
  colors), lty = rep(1, 7))

# Выбор наилучшей модели на основе критерия
# Акаике
...
# Сравнение наилучшей модели с нормальным
# распределением
...
# Тест Колмогорова-Смирнова для выбранной
# модели и нормального распределения
ks.test(rts.ret, rghyp(n = 100 * length(rts.ret), object = rts.best))
ks.test(rts.ret, rghyp(n = 100 * length(rts.ret), object = rts.gauss))

# ОЦЕНКИ РИСКА МЕТОДОМ МОНТЕ-КАРЛО НА ОСНОВЕ
# НАИЛУЧШЕЙ МОДЕЛИ ####

# Параметры расчета VaR и ES
conf.level <- 0.9  # доверительный уровень VaR
...
print(paste0("VaR1 best model/gauss: ", round(VaR1.best, 4), "/", round(VaR1.gauss, 
  4)))
print(paste0("VaR2 best model/gauss: ", round(VaR2.best, 4), "/", round(VaR2.gauss, 
  4)))
print(paste0("ES best model/gauss: ", round(ES.best, 4), "/", round(ES.gauss, 4)))
...
legend("bottomleft", c("Returns", "VaR hyp", "VaR gauss"), col = c("black", "blue", 
  "red"), lty = rep(1, 3))
...
print(paste0("hyp: alpha0 = ", round(kup[1], 6), ", p-value  = ", round(kup[2], 6)))
kup <- kupiec.test(rts.test, VaR.gauss, alpha)
print(paste0("gauss: alpha0 = ", round(kup[1], 6), ", p-value  = ", round(kup[2], 
  6)))
# Сравнение глубины пробоев
print(paste0("Lopez loss function hyp/gauss: ", round(lopez.lf(rts.test, VaR.hyp), 
  6), "/", round(lopez.lf(rts.test, VaR.gauss), 6)))
print(paste0("Blanco loss function hyp/gauss: ", round(blanco.lf(rts.test, VaR.hyp), 
  6), "/", round(blanco.lf(rts.test, VaR.gauss), 6))) 
