...
# Applied econometrics 8, Multivariate extreme value theory Орлов
# Евгений, 19.01.2015
...
for (i in (T1 + 1):(T1 + T2)) {
  ...
  # gal.cop.h <- galambosCopula(2) Значения частных функкций
  # распределения
  ...
  # gal.fit.h <- try(fitCopula(cdf.h, copula=gal.cop.h)) if (class(gal.fit.h) ==
  # 'try-error') { gal.fit.h <- fitCopula(cdf.h, copula=gal.cop.h, method='itau') }
  cdf.sim.h <- rCopula(n = N, copula = gumb.fit.h@copula)
  # Модельные значения максим
  sim1.h <- qgev(cdf.sim.h[, 1], loc = fit1.h$estimate[1], scale = fit1.h$estimate[2], 
    shape = fit1.h$estimate[3])
  sim2.h <- qgev(cdf.sim.h[, 2], loc = fit2.h$estimate[1], scale = fit2.h$estimate[2], 
    shape = fit2.h$estimate[3])
  # Модельные убытки портфеля
  loss.h <- sort(w[1] * sim1.h + w[2] * sim2.h)
  # VaR для данного шага
  VaR.bm[i - T1] <- loss.h[alpha * N]
}
# График кривой VaR
port.test <- w[1] * gazp.ret[(T1 + 1):(T1 + T2)] + w[2] * gmkn.ret[(T1 + 1):(T1 + 
  T2)]
plot(port.test, type = "l")
lines(-VaR.bm, col = "red")
...
print(paste0("Kupiec test, alpha = ", 1/260))
kup <- kupiec.test(port.test, -VaR.bm, 1/260)
print(paste0("Block Maximas: alpha0 = ", round(kup[1], 6), ", p-value  = ", round(kup[2], 
  6)))


# МЕТОД ПРЕВЫШЕНИЯ ПОРОГА ####

# Выборка значений, превышающих многомерный
# порог
thresh <- 0.9
...
for (i in (T1 + 1):(T1 + T2)) {
  ...
  # Частные распределения на основе GED
  fit1.h <- fpot(h.t.losses[, 1], threshold = h.u[1], model = "gpd", method = "SANN", 
    std.err = FALSE)
  fit2.h <- fpot(h.t.losses[, 2], threshold = h.u[2], model = "gpd", method = "SANN", 
    std.err = FALSE)
  # Значения частных функций распределения
  cdf1.h <- pgpd(h.t.losses[, 1], loc = h.u[1], scale = fit1.h$par[1], shape = fit1.h$par[2])
  cdf2.h <- pgpd(h.t.losses[, 2], loc = h.u[2], scale = fit2.h$par[1], shape = fit2.h$par[2])
  cdf.h <- cbind(cdf1.h, cdf2.h)
  # Подгонка копулы
  ...
  sim1.h <- qgpd(cdf.sim.h[, 1], loc = h.u[1], scale = fit1.h$par[1], shape = fit1.h$par[2])
  sim2.h <- qgpd(cdf.sim.h[, 2], loc = h.u[2], scale = fit2.h$par[1], shape = fit2.h$par[2])
  # Убытки по портфелю
  loss.h <- sort(w[1] * sim1.h + w[2] * sim2.h)
  # Расчет VaR
...
# Тест VaR-кривой на частоту пробоев
print(paste0("Kupiec test, alpha = ", 1/260))
kup <- kupiec.test(port.test, -VaR.gpd, 1/260)
